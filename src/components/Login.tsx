...
const Login: React.FC = () => {
    ...
    useEffect(() => {
        if (isAuthenticated && user) {
            const redirectPath = user.position === 'admin' ? '/admin/home' : '/user/home';
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const credentials = isAdmin ? formData.userName : formData.email;

        try {
            await login(credentials, formData.password, isAdmin);
        } catch (err) {
            setError(`${isAdmin ? 'Admin' : 'User'} đăng nhập thất bại!`);
        }
    };
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 px-4 py-8">
            <div className="w-full max-w-md bg-white shadow-2xl rounded-2xl overflow-hidden">
                <div className="border-b bg-gray-200 flex">
                    <button
                        onClick={() => setActiveTab('user')}
                        className={`flex-1 py-2 text-center text-sm font-medium ${
                            activeTab === 'user' ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-500'
                        }`}
                    >
                        Đăng nhập
                    </button>
                    <button
                        ...
                        className={`flex-1 py-2 text-center text-sm font-medium ${
                            activeTab === 'admin' ? 'text-green-600 border-b-2 border-green-600' : 'text-gray-500'
                        }`}
                    >
                        Đăng nhập Admin
                    </button>
                ...
                <div className="p-8 md:p-10">
                    <h2 className="text-2xl font-bold text-center text-gray-800 mb-6">
                        {activeTab === 'user' ? 'Đăng nhập' : 'Đăng nhập tài khoản Admin'}
                    </h2>
                    {error && (
                        ...
                        <div>
                            ...
                            >
                                {loading ? (
                                    ...
                                    activeTab === 'user' ? 'Đăng nhập User' : 'Đăng nhập Admin'
                                )}
                            </button>
                        </div>
                    </form>
